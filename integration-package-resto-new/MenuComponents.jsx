...
export const AnnouncementsSection = ({ menuSDK }) => {
  ...
  // Auto-avance de anuncios cada 5 segundos
  useEffect(() => {
  ...
  const handleImageError = (announcementId, imageIndex) => {
    setImageLoadErrors(prev => new Set([...prev, `${announcementId}-${imageIndex}`]));
  };

  const handleAnnouncementClick = (announcement) => {
    if (announcement.url && announcement.url.trim()) {
  ...
  if (error) {
    ...
    return null; // No mostrar error al usuario final
  }

  if (!announcements.length) {
    return null; // No hay anuncios para mostrar
  }
            ...
            {announcement.images && announcement.images.length > 0 && (
              <div className="announcement-images">
                {announcement.images.map((imageUrl, imageIndex) => {
                  const errorKey = `${announcement.id}-${imageIndex}`;
                  if (imageLoadErrors.has(errorKey)) return null;

                  return (
                    <img 
                      key={imageIndex}
                      src={imageUrl}
                      alt={`${announcement.title || 'Anuncio'} - imagen ${imageIndex + 1}`}
                      className="announcement-image"
                      onError={() => handleImageError(announcement.id, imageIndex)}
                      loading="lazy"
                    />
                  );
                })}
            ...
            <div className="announcement-content">
              ...
              {announcement.urlText && announcement.url && (
                <div className="announcement-link">
                  <span className="link-text">{announcement.urlText}</span>
